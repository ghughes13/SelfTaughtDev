...
export default () => {
  ...
  const updateRoles = ({ add, remove }) => {
    setProcessing(true)
    identity
      .authorizedFetch("/api/update-role", {
        method: "POST",
        body: JSON.stringify({
      ...
      <main className="max-w-2xl flex-grow mx-auto flex flex-col justify-around">
        <div className="sm:flex sm:flex-row-reverse sm:items-center">
          <div className="sm:px-2">
            ...
            <p className="px-4 mt-8 text-xl text-gray-700 sm:mt-8">
              Feel free to sign up for a user account and play around. Nothing
              on this demo site is tracked or monitored, and no emails are
              retained beyond Netlify Identity.
            </p>
            <p className="px-4 mt-8 text-xl text-gray-700 sm:mt-8">
              This site fully implements Netlify Identity - creating new users,
              exposing workflow steps to complete registrations, implementing
              recovery emails and invite tokens, and exposing important
              data-points in React.{" "}
              <code className="text-pink-800">
                gatsby-plugin-netlify-identity-gotrue
              </code>{" "}
              is a Gatsby-specific wrapper for{" "}
              <code className="text-pink-800">
                react-netlify-identity-gotrue
              </code>
              , which is written exclusively in React (Hooks) and carries{" "}
              <em>no</em> dependencies.
            </p>
            {identity.user && (
              <div className="pt-8 flex justify-around">
                {!identity.user.app_metadata?.roles?.includes("member") && (
                  <button
                    className={`bg-blue-500 text-white p-2 m-2 rounded text-m font-bold ${
